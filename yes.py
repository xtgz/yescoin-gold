# -*- coding: utf-8 -*-
# Made with ❤
# Join tele channel for update t.me/alloffmovie
...
def process_account(token):
    ...
    if squad_info and squad_info['data']['isJoinSquad']:
        ...
        print(f"{Fore.YELLOW + Style.BRIGHT}\r[ Squad ] : Belum Join Squad. Joining alloffmovie.", end="", flush=True)
        time.sleep(3)
        join_result = join_squad(token, "t.me/alloffmovie")
        if join_result:
            print(f"{random.choice(available_colors) + Style.BRIGHT}\r[ Squad ] : Welcome to The Jungle {nickname} - alloffmovie !      ", flush=True)
        else:
            print(f"{random.choice(available_colors) + Style.BRIGHT}\r[ Squad ] : Failed to join alloffmovie.", flush=True)
...
def parse_arguments():
    ...
    if args.task is None:
        task_input = input("task? (y/n, default n): ").strip().lower()
        args.task = task_input if task_input in ['y', 'n'] else 'n'
    
    if args.multi is None:
        multi_input = input("multi? (y/n, default n): ").strip().lower()
        args.multi = multi_input if multi_input in ['y', 'n'] else 'n'
    
    if args.fill is None:
        fill_input = input("fill? (y/n, default n): ").strip().lower()
